    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form action="{{ route('email-campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data"
                      x-data="{
                        form: {
                ...
                @if(!$campaign->isSent())
                <form id="delete-form" action="{{ route('email-campaigns.destroy', $campaign) }}" method="POST" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
                @endif
