...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
            ...
            <p class="text-gray-600">Manage your product categories and their organization</p>
        </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg" x-data="{
        ...
        init() {
            ...
            this.$watch('selectAll', value => {
                this.selectedCategories = value ? @json($categories->pluck('id')->toArray()) : [];
            });
        },
        ...
        toggleCategory(categoryId) {
            ...
            this.selectAll = this.selectedCategories.length === @json($categories->count());
        },
        
        executeBulkAction() {
            if (!this.bulkAction || this.selectedCategories.length === 0) return;
            
            if (this.bulkAction === 'delete' && !confirm('Are you sure you want to delete the selected categories? This action cannot be undone.')) {
                return;
            }
            
            // Submit the form with the selected categories and action
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("inventory.categories.bulk") }}';
            ...
            methodInput.value = this.bulkAction === 'delete' ? 'DELETE' : 'PATCH';
        ...
        <div x-show="selectedCount > 0" 
             ...
             class="bg-blue-50 border-b border-blue-200 px-4 py-3">
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-2">
                    ...
                    <button @click="selectedCategories = []; selectAll = false;"
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                <div class="w-full sm:w-auto mb-4 sm:mb-0">
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="text" 
                               x-model="searchQuery" 
                               @keyup.debounce.500ms="filterCategories"
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-b-lg">
                <ul class="divide-y divide-gray-200">
                    @foreach ($categories as $category)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span x-data="{ status: {{ $category->is_active ? 'true' : 'false' }} }"
                                          @click.self="status = !status; 
                                              fetch('{{ route('inventory.categories.update', $category) }}', {
                                                  method: 'POST',
                                                  headers: {
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('categories', () => ({
            ...
            confirmDelete(categoryId) {
                ...
                this.deleteUrl = `/inventory/categories/${categoryId}`;
                
                // Fetch product count for this category
                fetch(`/api/categories/${categoryId}/products/count`)
