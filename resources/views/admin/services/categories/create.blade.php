...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center mb-6">
        ...
        <h1 class="text-3xl font-bold">Create Service Category</h1>
    </div>
        ...
        <form action="{{ route('admin.services.categories.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input id="display_order" type="number" class="form-input w-full rounded-md shadow-sm @error('display_order') border-red-500 @enderror"
                    name="display_order" value="{{ old('display_order') }}" min="1">
                <p class="text-gray-500 text-xs mt-1">Leave empty to automatically assign the next available order number.</p>
                @error('display_order')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" class="form-checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }}>
